...
class AudioProcessor:
    ...
    def create_wav(self, wav):
        """Generate a wav file with header from signal """
        def genHeader(data) -> bytes:
            """ Create wav header for input data.

            Keyword Arguments: 
            ==================
            data (bytes) -- signal to create the header for.
            """
            channels = 1
            bitsPerSample = 16
            datasize = len(data) * channels * bitsPerSample // 8
            ...
            o += (self.sample_rate).to_bytes(4,'little')
            o += (self.sample_rate * channels * bitsPerSample // 8).to_bytes(4,'little')
        ...
        wav_file = genHeader(wav_norm)
        wav_file += wav_norm.tobytes()
        return wav_file
